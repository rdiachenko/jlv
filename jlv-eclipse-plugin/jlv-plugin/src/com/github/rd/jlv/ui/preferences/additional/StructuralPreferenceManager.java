...
public class StructuralPreferenceManager {
	...
	public StructuralPreferenceManager(IPreferenceStore store, String preferenceName) {
		this.store = store;
		this.preferenceName = preferenceName;
	}

	public StructuralPreferenceModel[] loadStructure() {
	...
	public StructuralPreferenceModel[] loadStructure(String structure) {
		return stringToStructure(structure);
	}

	public StructuralPreferenceModel[] loadDefaultStructure() {
		String prefs = store.getDefaultString(preferenceName);
		return stringToStructure(prefs);
	}

	public void storeTableStructure(StructuralPreferenceModel[] structure) {
		String prefs = structureToString(structure);
		store.setValue(preferenceName, prefs);
	}

	private StructuralPreferenceModel[] stringToStructure(String prefs) {
		String[] structureItems = prefs.split(SEMICOLUMN_SEPARATOR);
		StructuralPreferenceModel[] structure = new StructuralPreferenceModel[structureItems.length];
		...
		for (int i = 0; i < structure.length; i++) {
			...
			structure[i] = new StructuralPreferenceModel(name, width, display);
		}
		return structure;
	}

	private String structureToString(StructuralPreferenceModel[] structure) {
		StringBuilder builder = new StringBuilder();

		for (StructuralPreferenceModel structureItem : structure) {
